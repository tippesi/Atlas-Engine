...
void main() {
	...
	float k = mod(coord.y * float(N) / pow(2.0, coord.x + 1), N);
	...
	int span = int(pow(2.0, coord.x));

	bool topWing = mod(coord.y, pow(2.0, coord.x + 1)) < pow(2.0, coord.x);
	...
	if (coord.x == 0) {
		if (topWing)
			twiddleIndices = vec4(twiddleFactor.real, twiddleFactor.imag,
				bitrevIndices.index[int(coord.y)], bitrevIndices.index[int(coord.y + 1)]);
		else
			twiddleIndices = vec4(twiddleFactor.real, twiddleFactor.imag,
				bitrevIndices.index[int(coord.y - 1)], bitrevIndices.index[int(coord.y)]);
	}
	else {
		if (topWing)
			twiddleIndices = vec4(twiddleFactor.real, twiddleFactor.imag,
