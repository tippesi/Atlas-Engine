...
namespace Atlas {
    ...
    namespace Lighting {
        ...
        public:
            ...
            void AddVolumetric(int32_t sampleCount = 10, float intensity = 1.0f);

            void RemoveVolumetric() override;

            void Update(const CameraComponent& camera) override;

            vec3 direction = vec3(0.0f, -1.0f, 0.0f);

        private:
            void UpdateShadowCascade(ShadowComponent* cascade, const CameraComponent& camera);
