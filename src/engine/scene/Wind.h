    ...
    namespace Scene {
        ...
        public:
            Wind() {
                ...
                std::vector<float> amplitudes = {1.0f, 1.0f, 1.0f, 1.0f, 1.0f / 2.0f, 1.0f / 4.0f,
                                                 1.0f / 8.0f, 1.0f / 16.0f, 1.f / 32.0f};
                Common::NoiseGenerator::GeneratePerlinNoise2D(*image, amplitudes, 51234543, 2.0f);

                noiseMap = Texture::Texture2D(image, Texture::Wrapping::Repeat, Texture::Filtering::Linear);
