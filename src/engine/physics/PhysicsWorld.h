    ...
    namespace Physics {
        ...
        public:
            PhysicsWorld(uint32_t maxBodyCount = 65536, uint32_t bodyMutexesCount = 0, uint32_t maxBodyPairCount= 65536,
                uint32_t maxContactConstraintCount = 10240, Ref<JPH::ObjectLayerPairFilter> objectLayerFilter = nullptr,
        ...
        private:
            void MatrixToPosAndRot(const mat4& matrix, JPH::Vec3& pos, JPH::Quat& quat);
            void PosAndRotToMatrix(const JPH::Vec3& pos, const JPH::Quat& quat, mat4& matrix);
