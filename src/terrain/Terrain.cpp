...
Terrain::Terrain(int32_t rootNodeCount, int32_t LODCount, int32_t patchSize) : rootNodeCount(rootNodeCount), patchSize(patchSize) {
	...
	if (nodesCount > powl(2, 16)) {
		...
		this->LODCount = floorl(logl(3 * powl(2, 16) / rootNodeCount + 1) / log(4));
	}
	else {
